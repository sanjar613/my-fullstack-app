...
export async function renderRegisterPage() {
  ...
  // Add global function to toggle business fields
  window.toggleBusinessFields = (show) => {
    const businessFields = document.getElementById("businessFields");
    const businessInputs = businessFields.querySelectorAll("input, select");
...
function validateName(e) {
  ...
  if (name && name.length < 2) {
    ...
    if (
      ...
      !e.target.nextElementSibling.classList.contains("invalid-feedback")
    ) {
      const invalidFeedback = document.createElement("div");
      invalidFeedback.classList.add("invalid-feedback");
      invalidFeedback.innerText = "Name must be at least 2 characters long.";
